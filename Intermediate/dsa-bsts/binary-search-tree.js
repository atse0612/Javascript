...
class BinarySearchTree {
  ...
  insert(val) {
    if (this.root === null) { //assuming that the tree is null and empty
    ...
    while (true) {
      if (val < current.val) {
        if (current.left === null) {
          ...
          current = current.left; //find the correct spot for the new node on the left
        }
        ...
        if (current.right === null) {
          ...
          current = current.right; //find the correct spot for the new node on the right
        }
      }
    }
  }
  ...
  insertRecursively(val, current = this.root) {
    if (this.root === null) { //empty tree to inser the node 
      this.root = new Node(val);
      return this;
    }
  ...
  findRecursively(val, current = this.root) {
    ...
    if (val < current.val) {
      ...
      return this.findRecursively(val, current.left); //getting the left node
    } else if (val > current.val) {
      if (current.right === null) return undefined;
      return this.findRecursively(val, current.right); //getting the right node
    }
    return current;
  }
  ...
  dfsPreOrder() {
    let traveledTraverse = [];
    let current = this.root;

    function traverse(node) {
      traveledTraverse.push(node.val); // pushing the node
      node.left && traverse(node.left); //left node
      node.right && traverse(node.right); //right node
    }

    traverse(current);
    return traveledTraverse;
  }
  ...
  dfsPostOrder() {
    ...
    function traverse(node) {
      ...
      traveledTraverse.push(node.val); // visit
    }
  ...
  dfsInOrderIterative() { //depth first search
    let cur = this.root;
    let stack = [];
    let dfs = [];
